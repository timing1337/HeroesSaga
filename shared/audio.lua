...
function audio.preloadMusic(filename)
    if not isEnabled then return end
    if DEBUG > 1 then
        cclog("audio.preloadMusic() - filename: %s", tostring(filename))
    end
...
function audio.playMusic(filename, isLoop)
    ...
    if DEBUG > 1 then
        cclog("audio.playMusic() - filename: %s, isLoop: %s", tostring(filename), tostring(isLoop))
    end
...
function audio.playSound(filename, isLoop)
    ...
    if DEBUG > 1 then
        cclog("audio.playSound() - filename: %s, isLoop: %s", tostring(filename), tostring(isLoop))
    end
...
function audio.preloadSound(filename)
    if not isEnabled then return end
    if DEBUG > 1 then
        cclog("audio.preloadSound() - filename: %s", tostring(filename))
    end
