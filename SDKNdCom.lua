...
function SDKNdCom.init()
    ...
    local function callback(event)
        print("## SDKNdCom CALLBACK, event %s", tostring(event))
...
function SDKNdCom.showBar()
    return luaoc.callStaticMethod(SDK_CLASS_NAME, "showBar")
end
...
function SDKNdCom.messageBox(messageInfo,messageState)
...
function SDKNdCom.payForCoins(coins)
    onEnterPlatform()
    local args = {coins = tonumber(coins)}
...
function SDKNdCom.uniPayAsyn(cooOrderSerial,productId,productPrice,productName,productOrignalPrice,productCount,payDescription)
    onEnterPlatform()
    local args = {}
    args["cooOrderSerial"] = cooOrderSerial
    args["productId"] = productId
    args["productPrice"] = productPrice
    args["productName"] = productName
    args["productOrignalPrice"] = productOrignalPrice
    args["productCount"] = productCount
    args["payDescription"] = payDescription
    local ok, ret = luaoc.callStaticMethod(SDK_CLASS_NAME, "uniPayAsyn", args)
    if not ok then onLeavePlatform() end
    return ret
end
...
function SDKNdCom.share(message, image)
    ...
    if not ok then onLeavePlatform() end
    assert(ok, string.format("SDKNdCom.share() - call API failure, error code: %s", tostring(ret)))
end
