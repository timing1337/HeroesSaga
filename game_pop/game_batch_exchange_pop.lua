...
function game_batch_exchange_pop.destroy(self)
    -- body
    cclog("-----------------game_batch_exchange_pop destroy-----------------");
    self.m_popUi = nil;
    self.m_tParams = nil;
...
function game_batch_exchange_pop.back(self,type)
    game_scene:removePopByName("game_batch_exchange_pop");
end
--[[
    修改个数和钻石    从竞技场进来，使用的是竞技点
]]--
function game_batch_exchange_pop.setCountAndMoney(self,addCount)
...
function game_batch_exchange_pop.setCountAndArenaCount(self,addCount)
    -- body
end
--[[--
    读取ccbi创建ui
]]
function game_batch_exchange_pop.createUi(self)
    ...
    local function onMainBtnClick( target,event )
        ...
        elseif btnTag == 105 then--  购买
            cclog("count === " .. self.m_count_value)
    ...
    game_util:setCCControlButtonTitle(self.m_btn_use,"兑换")
    ...
    local icon = game_util:createItemIconByCfg(self.itemData.item_item_cfg)
    if icon then
        ...
        self.m_goods_info_label:setString(self.itemData.itemName)
    end
...
function game_batch_exchange_pop.refreshUi(self)
    
end
--[[--
    初始化
]]
function game_batch_exchange_pop.init(self,t_params)
...
function game_batch_exchange_pop.create(self,t_params)
    self:init(t_params);
...
function game_batch_exchange_pop.callBackFunc(self,typeName,t_params)
...
return game_batch_exchange_pop;