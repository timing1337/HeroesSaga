...
function game_guild_post_pop.destroy(self)
    -- body
    cclog("-----------------game_guild_post_pop destroy-----------------");
    ...
    self.m_popUi = nil;
    self.m_post_label = nil;
end
--[[--
    返回
]]
function game_guild_post_pop.back(self,backType)
    -- if self.m_popUi then
    --     self.m_popUi:removeFromParentAndCleanup(true);
    --     self.m_popUi = nil;
    -- end
    -- self:destroy();
    game_scene:removePopByName("game_guild_post_pop");
end
--[[--
    读取ccbi创建ui
]]
function game_guild_post_pop.createUi(self)
    ...
    local function onMainBtnClick( target,event )
        ...
        elseif btnTag == 11 then--会长
            if self.m_selPlayerJobType == 1 then require("game_ui.game_pop_up_box").showAlertView(string_helper.game_guild_post_pop.gvging); return end
        ...
        elseif btnTag == 14 then--移出公会
            if self.m_selPlayerJobType == 1 then require("game_ui.game_pop_up_box").showAlertView(string_helper.game_guild_post_pop.gvged); return end
            local function responseMethod(tag,gameData)
                game_data:setSelGuildDataByJsonData(gameData:getNodeWithKey("data"));
                game_scene:refreshCurrentUi();
                self.m_menu_node:setVisible(false)
    ...
    self.m_post_label = tolua.cast(ccbNode:objectForName("m_post_label"), "CCLabelTTF");
    self.m_menu_node:setVisible(false);
...
function game_guild_post_pop.initTeamFormation(self,formation_layer)
...
function game_guild_post_pop.refreshUi(self)
    if self.m_tGameData then
        ...
        if self.m_jobType == 1 then
            self.m_post_label:setString(string_helper.game_guild_post_pop.gvg_pr)
            self.m_post_btn:setVisible(true);
        elseif self.m_jobType == 2 then
            self.m_post_label:setString(string_helper.game_guild_post_pop .gvg_fr)
            self.m_post_btn:setVisible(true);
        elseif self.m_jobType == 3 then
            self.m_post_label:setString(string_helper.game_guild_post_pop .dy)
            self.m_post_btn:setVisible(false);
        end
    end
end
...
function game_guild_post_pop.init(self,t_params)
...
function game_guild_post_pop.create(self,t_params)
    self:init(t_params);
