...
function game_split_pop.destroy(self)
    -- body
    cclog("-----------------game_split_pop destroy-----------------");
    self.m_popUi = nil;
...
function game_split_pop.back(self,type)
 --    if self.m_popUi then
 --        self.m_popUi:removeFromParentAndCleanup(true);
 --        self.m_popUi = nil;
 --    end
	-- self:destroy();
    game_scene:removePopByName("game_split_pop");
end
--[[--
    读取ccbi创建ui
]]
function game_split_pop.createUi(self)
    ...
    self.m_title_label:setString(string_helper.game_split_pop.autoAdd)
...
function game_split_pop.auto_add(self,tag)
    ...
    if #showHeroTable == 0 then
        game_util:addMoveTips({text = string_helper.game_split_pop.cardDips});
    else
        self:back()
    end
end
...
function game_split_pop.refreshUi(self)
    self.m_tips_label_1:setString(string_helper.game_split_pop.sellCardTips1)
    self.m_tips_label_2:setString(string_helper.game_split_pop.sellCardTips2)
    self.m_tips_label_3:setString(string_helper.game_split_pop.sellCardTips3)
end

--[[--
    初始化
]]
function game_split_pop.init(self,t_params)
...
function game_split_pop.create(self,t_params)
    self:init(t_params);
...
function game_split_pop.callBackFunc(self,typeName,t_params)
    local callBackFunc = self.m_tParams.callBackFunc;
    if callBackFunc and type(callBackFunc) == "function" then
