...
function game_equip_select_pop.destroy(self)
    -- body
    cclog("-----------------game_equip_select_pop destroy-----------------");
...
function game_equip_select_pop.back(self,type)
...
function game_equip_select_pop.createUi(self)
    ...
    self.m_title_label:setString(string_helper.game_equip_select_pop.equipList)
...
function game_equip_select_pop.createEquipTableView(self,viewSize,tableData)
    ...
    params.newCell = function(tableView,index) 
        ...
        if cell then
            ...
            if index < itemsCount then
                ...
                local itemData,itemCfg = game_data:getEquipDataById(tableData[index+1]);
...
function game_equip_select_pop.createEquipTableView2(self,viewSize,tableData)
    cclog("game_equip_select_pop json.encode(tableData) == " .. json.encode(tableData))
        ...
        if cell then
            ...
            if index < itemsCount then
                if index == 0 then
                    ...
                    m_spr_bg_up:setVisible(false);
                    local itemData,itemCfg = game_data:getEquipDataById(tableData[index]);
                    game_util:setEquipItemInfoByTable(ccbNode,itemData);
                    local m_team_img = ccbNode:spriteForName("m_team_img")
                    if not m_team_img:isVisible() then
                        local id = game_guide_controller:getCurrentId();
    ...
    params.itemOnClick = function(eventType,index,item)
        ...
        if eventType == "ended" and item then
            if index == 0 then
                ...
                if game_data.getGuideProcess and game_data:getGuideProcess() == "guide_second" then
                    if game_util.statisticsSendUserStep then game_util:statisticsSendUserStep(50) end -- 新手引导33步 步骤50
                end
                local itemData,itemCfg = game_data:getEquipDataById(tableData[index]);
...
function game_equip_select_pop.refreshEquipTableView(self,sort)
    self.m_selSort = sort;
    if self.m_tableView ~= nil then
        self.m_tableView:removeFromParentAndCleanup(true);
    end
    local equipData = game_data:getEquipDataByEquipType(self.m_selSort);   
    if self.m_openType == 2 then 
        self.m_tableView = self:createEquipTableView2(self.m_list_view_bg:getContentSize(),equipData);
    else
    ...
    local typeName = EQUIP_TYPE_NAME_TABLE["type_" .. self.m_selSort]
    if typeName then
        self.m_title_label:setString(typeName .. string_helper.game_equip_select_pop.list)
    end
end
...
function game_equip_select_pop.refreshUi(self)
...
function game_equip_select_pop.init(self,t_params)
...
function game_equip_select_pop.create(self,t_params)
