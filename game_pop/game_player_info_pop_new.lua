...
local game_player_info_pop_new = {
   m_params = nil,
   m_progress_bg = nil,
   m_stageTab = nil,
...
function game_player_info_pop_new.destroy(self)
    -- body
    cclog("-----------------game_player_info_pop_new destroy-----------------");
    self.m_params = nil;
...
function game_player_info_pop_new.back(self,type)
        -- local function endCallFunc()
        --     self:destroy();
        -- end
        -- game_scene:enterGameUi("game_main_scene",{gameData = nil},{endCallFunc = endCallFunc});
    game_scene:removePopByName("game_player_info_pop_new");
end

--[[--
    读取ccbi创建ui
]]
function game_player_info_pop_new.createUi(self)
    ...
    local function onMainBtnClick( target,event )
        ...
        elseif btnTag == 102 then
            if self.m_uid then
                local function responseMethod(tag,gameData)
                    ...
                    game_util:addMoveTips({text = string_helper.game_player_info_pop_new.applyInfo});
                end
                local params = {};
        ...
        elseif btnTag == 103 then
            local logs = self.m_params.boss_data.logs
            local battle_log = logs.battle_log

            local params = {}
            params.battle_log = battle_log
            local function responseMethod(tag,gameData)
                game_data:setBattleType("open_door_cloister");
                game_scene:enterGameUi("game_battle_scene",{gameData = gameData});
            end
            network.sendHttpRequest(responseMethod,game_url.getUrlForKey("maze_battle_replay"), http_request_method.GET, params,"maze_battle_replay")
        ...
        elseif btnTag == 1005 then   -- 属性
            ...
            self:refreshUi()
        elseif btnTag == 1006 then --buff
            self.m_selRightIndex = 6
            self:refreshUi()
        else
        end
    end
    ccbNode:registerFunctionWithFuncName("onMainBtnClick",onMainBtnClick);
    ccbNode:openCCBFile("ccb/ui_newplayer_info_scene_buff.ccbi");
    ...
    self.m_node_iconboard6 = ccbNode:nodeForName("m_node_iconboard6")
    ...
    game_util:setCCControlButtonTitle(self.m_add_friend_btn,string_helper.ccb.title142)
    game_util:setCCControlButtonTitle(self.m_add_guild_btn,string_helper.ccb.title143)
    self.m_label_uid = ccbNode:labelTTFForName("m_label_uid")
    local title137 = ccbNode:labelTTFForName("title137");
    local title138 = ccbNode:labelTTFForName("title138");
    local title139 = ccbNode:labelTTFForName("title139");
    local title140 = ccbNode:labelTTFForName("title140");
    local title141 = ccbNode:labelTTFForName("title141");
    title137:setString(string_helper.ccb.title137);
    title138:setString(string_helper.ccb.title138);
    title139:setString(string_helper.ccb.title139);
    title140:setString(string_helper.ccb.title140);
    title141:setString(string_helper.ccb.title141);
    ...
    self.m_label_uid:setString(string_helper.game_player_info_pop_new.userId .. (self.m_uid or ""))
    ...
    if self.m_add_friend_btn:isVisible() == false then
        self.m_add_guild_btn:setPositionX(200)
    else
        self.m_add_guild_btn:setPositionX(300)
    end
...
function game_player_info_pop_new.updateTopPlayerBaseInfo( self )
...
function game_player_info_pop_new.initRightContentInfo( self, index )
...
function game_player_info_pop_new.showEquipInfo( self, roleIndex, index)
...
function game_player_info_pop_new.showGemInfo( self, roleIndex, index)
...
function game_player_info_pop_new.showBreakInfo( self, roleIndex, index )
    local cardData,cardCfg = self:getCardDataByPos(roleIndex)
    if cardData and cardCfg then
...
function game_player_info_pop_new.showSkillInfo( self, roleIndex, index )
    local cardData,cardCfg = self:getCardDataByPos(roleIndex)
...
function game_player_info_pop_new.getPosEquip( self, equipPos )
    local equipData = {}
...
function game_player_info_pop_new.getPosEquipId(self,equipPos,equipType)
    local tempEquipId = 0;
    local equipData = {}
    if self.m_selLeftIndex == 1 then
        equipData = self.m_tempEquipPosData
...
function game_player_info_pop_new.createTouchButton( self, fun, parent, tag , anchorX, anchorY)
...
function game_player_info_pop_new.refreshTabBtn(self)
    local tempBtn = nil;
    local tempNode = nil;
...
function game_player_info_pop_new.updateFromation( self )
...
function game_player_info_pop_new.initTeamFormation(self,formation_layer)
...
function game_player_info_pop_new.selIcon( self, index )
...
function game_player_info_pop_new.getCardDataByPos( self, posIndex, showType )
...
function game_player_info_pop_new.getTeamCardDataByPos(self,posIndex)
...
function game_player_info_pop_new:getFriendCardDataByPos( posIndex )
    -- body
...
function game_player_info_pop_new:getDestinyCardDataByPos( posIndex )
    -- body
...
function game_player_info_pop_new.createBoxAnim(self,animFile)
...
function game_player_info_pop_new.refreshUi(self)
   self:refreshTabBtn()
...
function game_player_info_pop_new.refreshRewardStutas( self )
...
function game_player_info_pop_new.init(self,t_params)
...
function game_player_info_pop_new.getCardInfo( self, cardId )
    -- body
end
...
function game_player_info_pop_new.create(self,t_params)
    -- body
    self:init(t_params);
...
function game_player_info_pop_new.createBlessingTableView(self,viewSize)
    local maze_buff_cfg = getConfig(game_config_field.maze_buff)
    local buffs = self.m_params.boss_data.buff
    local showData = {}
    ...
    params.itemOnClick = function(eventType,index,item)
        if eventType == "ended" and item then
            ...
            game_scene:addPop("game_maze_item_pop2",{itemData = {id = tempId},itemType = 2})
        end
    end
...
function game_player_info_pop_new.refreshBlessingTableView(self)
    self.m_node_iconboard6:removeAllChildrenWithCleanup(true);
    local tableView = self:createBlessingTableView(self.m_node_iconboard6:getContentSize());
    self.m_node_iconboard6:addChild(tableView,10,10);
end

return game_player_info_pop_new;
