--- game_maze_item_pop信息

local game_maze_item_pop = {
    m_popUi = nil,
    m_root_layer = nil,
    m_ccbNode = nil,
    m_callBackFunc = nil,
...
function game_maze_item_pop.destroy(self)
    -- body
    cclog("-----------------game_maze_item_pop destroy-----------------");
    self.m_popUi = nil;
...
function game_maze_item_pop.back(self,type)
    game_scene:removePopByName("game_maze_item_pop");
end
--[[--
    读取ccbi创建ui
]]
function game_maze_item_pop.createUi(self)
            ...
            if self.m_itemType == 1 then
                local function responseMethod(tag,gameData)
                    ...
                    if itemCfg then
                        ...
                        game_util:addMoveTips({text = tostring(name) .. string_helper.game_maze_item_pop.use});
                    end
    ...
    ccbNode:openCCBFile("ccb/ui_maze_item_pop.ccbi");
    ...
    game_util:setCCControlButtonTitle(m_left_btn,string_helper.ccb.title164)
    game_util:setCCControlButtonTitle(m_right_btn,string_helper.ccb.title165)
    ...
    if self.m_itemType == 1 then
        ...
        if itemCfg then
            ...
            if sort ~= 0 then
                ...
                game_util:setCCControlButtonTitle(m_left_btn,string_helper.game_maze_item_pop.use2);
            ...
            if story then
                ...
                story = string_helper.game_maze_item_pop.cfgTips
            end
    ...
    elseif self.m_itemType == 2 then
        ...
        if itemCfg then
            ...
            if story then
                ...
                story = string_helper.game_maze_item_pop.cfgTips2
            end
            m_story_label:setString(story);
        end
...
function game_maze_item_pop.refreshUi(self)
...
function game_maze_item_pop.init(self,t_params)
...
function game_maze_item_pop.create(self,t_params)
    self:init(t_params);
