...
function game_gvg_rules_pop.destroy(self)
    -- body
    cclog("----------------- game_gvg_rules_pop destroy-----------------");  
...
function game_gvg_rules_pop.back(self,backType)
    game_scene:removePopByName("game_gvg_rules_pop");
    self:destroy();
end
--[[--
    读取ccbi创建ui
]]
function game_gvg_rules_pop.createUi(self)
...
function game_gvg_rules_pop.updateRichLabel(self,showMsg)
...
function game_gvg_rules_pop.refreshDetailInfo(self, adver_Id)
    local annoucement_cfg = getConfig(game_config_field.adver_guild)
    ...
    local title = itemCfg:getNodeWithKey("title") and itemCfg:getNodeWithKey("title"):toStr() or string_helper.game_gvg_rules_pop.superHero;
    for i=1,3 do
...
function game_gvg_rules_pop.createMsgContent( self )
    ...
    for i=1, #tempData or 0 do
        ...
        if label then
            ...
            if richLabel then
                local adver_Id = tempData[i]
                local itemCfg = annoucement_cfg:getNodeWithKey(tostring(adver_Id))
                local title = itemCfg:getNodeWithKey("ad_title") and itemCfg:getNodeWithKey("ad_title"):toStr() or string_helper.game_gvg_rules_pop.superHero;
                y = y - 20
                for i= 1, 4 do
...
function game_gvg_rules_pop.resetBarPos( self, posY )
    -- body
end
...
function game_gvg_rules_pop.createOpeningTabelView(self)
...
function game_gvg_rules_pop.refreshUi(self)
    self:initData()
    self:createOpeningTabelView();
...
function game_gvg_rules_pop.init(self,t_params)
...
function game_gvg_rules_pop.initData( self )
...
function game_gvg_rules_pop.create(self,t_params)
    self:init(t_params);
...
function game_gvg_rules_pop.createActivityTableView( self, viewSize )
    ...
    params.newCell = function ( tableView, index )
        ...
        if cell then  
            ...
            -- 公告标题
            local title = string_helper.game_gvg_rules_pop.superHero;
            if itemCfg then
                title = itemCfg:getNodeWithKey("title") and itemCfg:getNodeWithKey("title"):toStr() or string_helper.game_gvg_rules_pop.superHero
            end
...
function game_gvg_rules_pop.initContentBar( self )
    local tempSize = self.m_node_slider_tboard:getContentSize()
    local sidebar = CCControlSlider:create()
end

return game_gvg_rules_pop;
