...
local game_fuli_subui_invitecode = {
    m_root_layer = nil,
    m_popUi = nil,
    m_list_view_bg = nil,
    m_tGameData = nil,
...
function game_fuli_subui_invitecode.destroy(self)
    -- body
    cclog("----------------- game_fuli_subui_invitecode destroy-----------------"); 
    self.m_popUi = nil;
...
function game_fuli_subui_invitecode.back(self,backType)
        self:destroy();
end
--[[--
    读取ccbi创建ui
]]
function game_fuli_subui_invitecode.createUi(self)
    ...
    local function onBtnClick( target,event )
        ...
        if btnTag == 2 then--拜师
            ...
            if self.m_inputUid == nil or  self.m_inputUid == "" then
                game_util:addMoveTips({text = string_helper.game_fuli_subui_invitecode.inputMaster});
                return;
            end
    ...
    ccbNode:openCCBFile("ccb/ui_fuli_content_invitecode.ccbi");
    ...
    self.m_no_reward_label:setString(string_helper.ccb.file86)
    ...
    game_util:setCCControlButtonTitle(self.m_ok_btn,string_helper.ccb.text177)
    ...
    self.m_editBox = game_util:createEditBox({bgFileName = nil,scriptEditBoxHandler=editBoxTextEventHandle,size = self.m_edit_bg_node:getContentSize(),placeHolder=string_helper.ccb.file51,maxLength = 20,fontSize = 12});
...
function game_fuli_subui_invitecode.createTableView(self,viewSize)
    local request_code = self.m_tGameData.request_code or {}
        ...
        if btnTag < 100 then
            ...
            local t_params = 
            {
                ...
                okBtnCallBack = function(target,event)
                    ...
                    if uid then
                        local function responseMethod(tag,gameData)
                            game_util:addMoveTips({text = string_helper.game_fuli_subui_invitecode.kickStudent});
                            local data = gameData:getNodeWithKey("data")
                            self.m_tGameData = json.decode(data:getFormatBuffer()) or {};
                            self:refreshData();
                            self:refreshUi();
    ...
    params.newCell = function(tableView,index) 
        ...
        if cell then
            ...
            game_util:setCCControlButtonTitle(m_remove_btn,string_helper.ccb.title199)
            ...
            m_no_reward_label:setString(string_helper.ccb.file86)
            ...
            if index < showCount then
                ...
                if itemData then
                    ...
                    m_level_label:setString(string.format(string_helper.game_fuli_subui_invitecode.level,tostring(itemData.level)));
...
function game_fuli_subui_invitecode.getRewardFlag(self,itemData)
...
function game_fuli_subui_invitecode.refreshTableView(self)
...
function game_fuli_subui_invitecode.refreshData(self)
    self.m_idTab = {};
    local request_code = self.m_tGameData.request_code or {}
    local slave = request_code.slave or {};
...
function game_fuli_subui_invitecode.refreshUi(self)
    ...
    for k,v in pairs(master) do
        ...
        self.m_level_label:setString(string.format(string_helper.game_fuli_subui_invitecode.level,tostring(v.level)));
    ...
    cclog2(masterFlag, "masterFlag  ====  ")
    if masterFlag == true then
        cclog2("self.m_input_node:setVisible(false)")
        self.m_input_node:setVisible(false);
        self.m_info_node:setVisible(true);
    else
        cclog2("self.m_input_node:setVisible(true)")
        self.m_input_node:setVisible(true);
        self.m_info_node:setVisible(false);
    end
    cclog2(self.m_input_node, "m_input_node  ====  ")
end

--[[--
    初始化
]]
function game_fuli_subui_invitecode.init(self,t_params)
...
function game_fuli_subui_invitecode.create(self,t_params)
    self:init(t_params);
...
return game_fuli_subui_invitecode