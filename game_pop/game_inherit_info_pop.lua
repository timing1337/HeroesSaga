...
function game_inherit_info_pop.destroy(self)
    -- body
    cclog("----------------- game_inherit_info_pop destroy-----------------");  
...
function game_inherit_info_pop.back(self,backType)
    game_scene:removePopByName("game_inherit_info_pop");
    self:destroy();
end
--[[--
    读取ccbi创建ui
]]
function game_inherit_info_pop.createUi(self)
    ...
    ccbNode:openCCBFile("ccb/ui_gvg_rules.ccbi");
    ...
    self.m_root_layer:registerScriptTouchHandler(onTouch,false,GLOBAL_TOUCH_PRIORITY - 14,true);
    self.m_root_layer:setTouchEnabled(true);
    -- 重置按钮出米优先级 防止被阻止
    self.m_close_btn = ccbNode:controlButtonForName("m_close_btn");
    self.m_close_btn:setTouchPriority(GLOBAL_TOUCH_PRIORITY - 15);
    ...
    self.m_scro_showinfo:setTouchPriority(GLOBAL_TOUCH_PRIORITY - 15);
    return ccbNode;
end

function game_inherit_info_pop.updateRichLabel(self,showMsg)
...
function game_inherit_info_pop.refreshDetailInfo(self, adver_Id)
    local annoucement_cfg = getConfig(game_config_field.adver_inheritance)
    ...
    local title = itemCfg:getNodeWithKey("title") and itemCfg:getNodeWithKey("title"):toStr() or string_helper.game_inherit_info_pop.title;
    for i=1,3 do
...
function game_inherit_info_pop.createMsgContent( self )
    ...
    for i=1, #tempData or 0 do
        ...
        if label then
            ...
            if richLabel then
                local adver_Id = tempData[i]
                local itemCfg = annoucement_cfg:getNodeWithKey(tostring(adver_Id))
                local title = itemCfg:getNodeWithKey("ad_title") and itemCfg:getNodeWithKey("ad_title"):toStr() or string_helper.game_inherit_info_pop.title;
                y = y - 20
                for i= 1, 4 do
...
function game_inherit_info_pop.resetBarPos( self, posY )
    -- body
end
...
function game_inherit_info_pop.createOpeningTabelView(self)
...
function game_inherit_info_pop.refreshUi(self)
    self:initData()
    self:createOpeningTabelView();
...
function game_inherit_info_pop.init(self,t_params)
...
function game_inherit_info_pop.initData( self )
...
function game_inherit_info_pop.create(self,t_params)
    self:init(t_params);
...
function game_inherit_info_pop.createActivityTableView( self, viewSize )
    ...
    params.touchPriority = GLOBAL_TOUCH_PRIORITY-15;
    ...
    params.newCell = function ( tableView, index )
        ...
        if cell then  
            ...
            local itemCfg = annoucement_cfg:getNodeWithKey(tostring(key))
            -- 公告标题
            local title = string_helper.game_inherit_info_pop.title;
            if itemCfg then
                title = itemCfg:getNodeWithKey("title") and itemCfg:getNodeWithKey("title"):toStr() or string_helper.game_inherit_info_pop.title
            end
...
function game_inherit_info_pop.initContentBar( self )
    local tempSize = self.m_node_slider_tboard:getContentSize()
    local sidebar = CCControlSlider:create()
end

return game_inherit_info_pop;
