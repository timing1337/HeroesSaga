...
function fund_explain_server_pop.destroy(self)
    -- body
    cclog("-----------------fund_explain_server_pop destroy-----------------");
    self.m_popUi = nil;
    self.m_fromUi = nil;
...
function fund_explain_server_pop.back(self,type)
    game_scene:removePopByName("fund_explain_server_pop");
end
--[[--
    读取ccbi创建ui
]]
function fund_explain_server_pop.createUi(self)
...
function fund_explain_server_pop.createTabelView(self,viewSize)
...
function fund_explain_server_pop.createTableView2(self,viewSize,rewardTab,showIndex)
...
function fund_explain_server_pop.refreshTableView(self)
...
function fund_explain_server_pop.refreshUi(self)
    self:refreshTableView();
    -- 详情
    local activeCfg = getConfig(game_config_field.server_inreview)
    local itemCfg = activeCfg:getNodeWithKey( "907" )
    local contentText = itemCfg and itemCfg:getNodeWithKey("story") and itemCfg:getNodeWithKey("story"):toStr() or string_helper.game_active_limit_detail_pop.defaultText
...
function fund_explain_server_pop.init(self,t_params)
...
function fund_explain_server_pop.create(self,t_params)
    self:init(t_params);
...
return fund_explain_server_pop;