...
local game_lucky_turntable_reward_server_pop = {
    m_root_layer = nil,
    m_popUi = nil,
    m_close_btn = nil,
    m_list_view_bg = nil,
...
function game_lucky_turntable_reward_server_pop.destroy(self)
    -- body
    cclog("----------------- game_lucky_turntable_reward_server_pop destroy-----------------"); 
...
function game_lucky_turntable_reward_server_pop.back(self,backType)
    game_scene:removePopByName("game_lucky_turntable_reward_server_pop");
end
--[[--
    读取ccbi创建ui
    icon_box4.png
]]
function game_lucky_turntable_reward_server_pop.createUi(self)
...
function game_lucky_turntable_reward_server_pop.createTableView(self,viewSize)
    local roulette_rank_reward_cfg = getConfig(game_config_field.server_roulette_rank_reward)
    local version = tostring(self.m_version or "");
    ...
    params.newCell = function(tableView,index) 
        ...
        if cell then
            ...
            if rankUp == rankDown then
                reward_label:setString(string_helper.game_lucky_turntable_reward_server_pop.di .. rankUp .. string_helper.game_lucky_turntable_reward_server_pop.ming)
            else
                reward_label:setString(string_helper.game_lucky_turntable_reward_server_pop.di .. rankUp .. "-" .. rankDown .. string_helper.game_lucky_turntable_reward_server_pop.ming)
            end
...
function game_lucky_turntable_reward_server_pop.createTableView2(self,viewSize,rewardTab)
...
function game_lucky_turntable_reward_server_pop.refreshTableView(self)
...
function game_lucky_turntable_reward_server_pop.refreshUi(self)
    self:refreshTableView();
end
--[[--
    初始化
]]
function game_lucky_turntable_reward_server_pop.init(self,t_params)
...
function game_lucky_turntable_reward_server_pop.create(self,t_params)
    self:init(t_params);
...
return game_lucky_turntable_reward_server_pop;
