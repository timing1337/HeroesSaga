---  押镖队伍招募
local game_dart_team_recruitment = {
    m_table_node= nil,
    m_root_layer = nil,
...
function game_dart_team_recruitment.destroy(self)
    cclog("----------------- game_dart_team_recruitment destroy-----------------"); 
...
function game_dart_team_recruitment.back(self)
...
function game_dart_team_recruitment.createUi(self)
    ...
    local function onBtnClick( target,event )
            ...
            if identity == 0 then
                ...
                if sel_dart_good then
                    local function responseMethod(tag,gameData)
                        ...
                        game_scene:enterGameUi("game_dart_my_team",{gameData = gameData})
                    end
                    local params = {}
                    ...
                    game_util:addMoveTips({text = string_helper.game_dart_team_recruitment.text})
                end
            else
                local function responseMethod(tag,gameData)
                    game_scene:enterGameUi("game_dart_my_team",{gameData = gameData})
        ...
        elseif btnTag == 3 then--刷新
            local function responseMethod(tag,gameData)
                ...
                self:refreshUi();
                game_util:addMoveTips({text = string_helper.game_dart_team_recruitment.refresh_success})
            end
        ...
        elseif btnTag == 11 then  -- 聊天
            game_scene:addPop("ui_chat_pop", {enterType = 4,  openType = 1})
        end
    end

    ccbNode:registerFunctionWithFuncName("onMainBtnClick", onBtnClick);
    ccbNode:openCCBFile("ccb/ui_dart_team_recruitment.ccbi");
    ...
    local m_conbtn_chat = ccbNode:controlButtonForName("m_conbtn_chat")
...
function game_dart_team_recruitment.createTeamInfoTable(self,viewSize)
    local teamInfoTable = self.m_tGameData.vehicle_info or {}
    local teamKey = {}
        ...
        if game_util:valueInTeam(uid,itemData.member) then
            ...
            if sel_dart_good then
                ...
                local function responseMethod(tag,gameData)
                    -- game_data:setDataByKeyAndValue("sel_dart_good",nil)
                    game_scene:enterGameUi("game_dart_my_team",{gameData = gameData})
                    game_util:addMoveTips({text = string_helper.game_dart_team_recruitment.add_team_success})
                end
    ...
    params.newCell = function(tableView,index) 
        ...
        if cell then
            ...
            local goods = itemData.goods
            local map_id = itemData.map_id
            local buff_sort = itemData.buff_sort
            local escort_time = itemData.escort_time
            local arrive_time = itemData.arrive_time
            ...
            if buff_sort_item then
                ...
                m_ship_sprite:setScale(buff_sort_item.scale)
                game_util:setNodeUpAndDownMoveAction(m_ship_sprite)
            end
        end
        return cell;
    end
...
function game_dart_team_recruitment.refreshUi(self)
...
function game_dart_team_recruitment.init(self,t_params)
...
function game_dart_team_recruitment.create(self,t_params)
    self:init(t_params);
...
return game_dart_team_recruitment;