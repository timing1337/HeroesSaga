...
function game_task_scene.destroy(self)
    -- body
    cclog("-----------------game_task_scene destroy-----------------");
    self.m_popUi = nil;
...
function game_task_scene.back(self,type)
...
function game_task_scene.createUi(self)
...
function game_task_scene.createRewardTableView(self,viewSize)
            ...
            if btnFlag then
                ...
                if game_util:valueInTeam(itemCfg:getKey(),reward) then
                    local function responseMethod(tag,gameData)
                        game_util:setCCControlButtonTitle(tagNode,string_helper.game_task_scene.sd_out)
    ...
    params.newCell = function(tableView,index) 
        ...
        if cell and itemCfg then
            ...
            if ccbNode then
                ...
                local m_btn = ccbNode:controlButtonForName("m_btn")
                m_top_label:setString(string_helper.game_task_scene.qd .. (index+1) .. string_helper.game_task_scene.day)
                ...
                if (index+1) <= score then
                    if not game_util:valueInTeam(itemCfg:getKey(),reward) then
                        ...
                        game_util:setCCControlButtonTitle(m_btn,string_helper.game_task_scene.sd_out)
                    else
                        m_btn:setVisible(true)
                        m_btn:setEnabled(true);
                        game_util:setCCControlButtonTitle(m_btn,string_helper.game_task_scene.reward)
                    end
                else
...
function game_task_scene.createDailyTaskTableView(self,viewSize)
...
function game_task_scene.refreshTab2(self)
...
function game_task_scene.createMainTaskTableView(self,viewSize)
    CCSpriteFrameCache:sharedSpriteFrameCache():addSpriteFramesWithFile("ccbResources/ui_vip.plist");
...
function game_task_scene.refreshTab3(self)
    -- self.m_tab_node_1:setVisible(false)
    self.m_tab_node_2_3:setVisible(true);
    -- self.m_tab_node_4:setVisible(false);
...
function game_task_scene.refreshTab4(self)
    -- self.m_tab_node_1:setVisible(false)
...
function game_task_scene.refreshTabBtn(self)
...
function game_task_scene.refreshUi(self)
    self:refreshTabBtn();    
end
...
function game_task_scene.init(self,t_params)
...
function game_task_scene.create(self,t_params)
    self:init(t_params);
