...
function game_exchange_newServer:destroy()
	cclog("-----------------game_exchange_newServer destroy------------------------------");
...
function game_exchange_newServer:back(backType)
...
function game_exchange_newServer:createUi()
...
function game_exchange_newServer:createTableView(viewSize)
	-- local lastcount =  {};
 ...
 --    network.sendHttpRequest(responseMethod,game_url.getUrlForKey("omni_exchange_lastcount"), http_request_method.GET, nil,"omni_exchange_lastcount")
	...
	local open_se_info = self.m_tGameData.open_se_info
	if open_se_info == nil then
	...
	local omni_exchange_cfg  = getConfig(game_config_field.server_exchange); 
	-- cclog2(omni_exchange_cfg,"omni_exchange_cfg====")
	...
	if omni_exchange_cfg  then
		...
		for i=1,tempCount do
			...
			local itemData = open_se_info[itemCfg:getKey()]
			local end_time = itemData.end_time
			local start_time = itemData.start_time
			...
			-- local m_typeid = itemCfg:getNodeWithKey("type"):toInt()
			-- if self.typeid == m_typeid then
				if exchangestate == 0 then
					...
					if tonumber(end_time) > server_time  and  tonumber(start_time) < server_time then
	                    ...
	                    elseif exchangcount > 0 then
	                        ...
	                        -- cclog2(tempOwnCount,"tempOwnCount")
		...
		local function onCellBtnClick( target,event )
			...
			if itemCfg then
				...
				for i=1,need_item_count do
					...
					-- cclog2(senddatalist,"===senddatalist===")
                ...
                local function exchageItem()
                    ...
                    network.sendHttpRequest(responseMethod,game_url.getUrlForKey("server_exchange"), http_request_method.GET, params,"server_exchange")
                end
                if cardTipsFlag == true then
                    local t_params = 
                    {
                        ...
                        text = string_helper.game_exchange_newServer.text,      --可缺省
                        onlyOneBtn = false,
                ...
                elseif equipTipsFlag == true then
                    local t_params = 
                    {
                        ...
                        okBtnText = string_config.m_btn_sure,       --可缺省
                        cancelBtnText = string_config.m_btn_cancel,
                        text = string_helper.game_exchange_newServer.text2,      --可缺省
                        onlyOneBtn = false,
                        touchPriority = GLOBAL_TOUCH_PRIORITY-2,
                    }
                    game_util:openAlertView(t_params)
	    ...
	    -- cclog2(self.m_showExchangeIdTab,"self.m_showExchangeIdTab")
	   ...
	   --          local start_time = itemCfg:getNodeWithKey("start_time"):toStr();
				-- local end_time = itemCfg:getNodeWithKey("end_time"):toStr();
				local start_time = open_se_info[itemCfg:getKey()].start_time
				local end_time = open_se_info[itemCfg:getKey()].end_time
				end_time = end_time == "" and "0" or end_time
				local show_idcount = itemCfg:getNodeWithKey("show_id"):toStr();
	     ...
	     --            	m_ccnodeitem:addChild(bgSpr);
	                    ...
	                    local tempLabel = game_util:createLabelTTF({text = string_helper.game_exchange_newServer.random_package,color = ccc3(250,180,0),fontSize = 10});
                        tempLabel:setPosition(ccp(exwidth,10));
...
local YPOINT = 0 ;
--在一个cell里添加触摸事件
function game_exchange_newServer:initLayerTouch(formation_layer)
    local function onTouch(eventType, x, y)
      	if eventType == "began" then
...
function game_exchange_newServer:refreshUi()
	local pX,pY;
...
function game_exchange_newServer:init(t_params)
...
function game_exchange_newServer:create(t_params)
	self:init(t_params);
...
return game_exchange_newServer;