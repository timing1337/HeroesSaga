...
function fund_main_scene.destroy(self)
    -- body
    cclog("-----------------fund_main_scene destroy-----------------");
...
function fund_main_scene.back(self,type)
    game_scene:enterGameUi("game_main_scene");
end
--[[--
    读取ccbi创建ui
]]
function fund_main_scene.createUi(self)
    ...
    local function onMainBtnClick( target,event )
        ...
        elseif btnTag == 102 then--激活
            ...
            if selItemCount == 0 then
                game_util:addMoveTips({text = string_helper.fund_main_scene.text})
                return;
            end
            local t_params = 
            {
                ...
                okBtnCallBack = function(target,event)
                    ...
                    network.sendHttpRequest(responseMethod,game_url.getUrlForKey("foundation_activate"), http_request_method.GET, params,"foundation_activate",true,true)
                    -- local screenShoot = game_util:createScreenShoot();
                ...
                okBtnText = string_helper.fund_main_scene.okBtnText,       --可缺省
                text = string_helper.fund_main_scene.text2 .. costCoin .. string_helper.fund_main_scene.text3,      --可缺省
                onlyOneBtn = false,
            }
    ...
    m_detail_label:setString(string_helper.ccb.text184)
    local text1 = ccbNode:labelTTFForName("text1")
    text1:setString(string_helper.ccb.text183)
    local text2 = ccbNode:labelTTFForName("text2")
    text2:setString(string_helper.ccb.text185)
    return ccbNode;
end
...
function fund_main_scene.setCountdownTime(self)
    ...
    local function timeEndFunc()
       self.m_left_time_node:removeAllChildrenWithCleanup(true)
       local tipsLabel = game_util:createLabelTTF({text = string_helper.fund_main_scene.text4,color = ccc3(0,255,0),fontSize = 10});
...
function fund_main_scene.refreshFund(self)
    local foundation_cfg = getConfig(game_config_field.foundation)
...
function fund_main_scene.refreshUi(self)
...
function fund_main_scene.init(self,t_params)
...
function fund_main_scene.create(self,t_params)
    -- body
    self:init(t_params);
