...
function game_buy_item_scene.destroy(self)
    -- body
    cclog("-----------------game_buy_item_scene destroy-----------------");
...
local shopSortTab = string_helper.game_buy_item_scene.shopSortTab
local needSortTab = string_helper.game_buy_item_scene.needSortTab

--[[--
    返回
]]
function game_buy_item_scene.back(self,backType)
...
function game_buy_item_scene.createUi(self)
    ...
    self.m_title_label:setString(string_helper.game_buy_item_scene.prop_buy)
...
function game_buy_item_scene.createTableView(self,viewSize)    
                        ...
                        if #shop_reward > 0 then
                            ...
                            if #shop_reward_item > 2 then
                                ...
                                if tempType == 6 then--道具
                                    ...
                                    local function responseMethod(tag,gameData)
                                        game_util:addMoveTips({text = string_helper.game_buy_item_scene.buy_success});
                                    end
                ...
                if itemCfg and itemData then
                    ...
                    if sell_sort == 1 then
                        if sell_max-bought == 0 then
                            ...
                            m_cdTime:setString(string_helper.game_buy_item_scene.day_refresh)
                        else
                    ...
                    elseif sell_sort == 2 then
                        if sell_max-bought == 0 then
                            cd_node:setVisible(true)
                            m_cdTime:setString(string_helper.game_buy_item_scene.week_refresh)
                        end
                    else--不限次数
                        cd_node:setVisible(false)
                    end
                ...
                if #shop_reward > 0 then
                    ...
                    if #shop_reward_item > 2 then
                        ...
                        if tempType == 6 then--道具
                            ...
                            game_scene:addPop("game_item_info_pop",{itemId = itemId,openType = 10,maxCount = self.buy_max_count,alreadyCount = self.already_count,tempId = tempId,buy_call_back = buy_call_back})
                        ...
                        elseif tempType == 5 then--卡牌
                            ...
                            game_scene:addPop("game_food_info_pop",{itemId = itemId,itemData = shop_reward_item,openType = 2,maxCount = self.buy_max_count,alreadyCount = self.already_count,tempId = tempId,buy_call_back = buy_call_back})
                        end
                    end
                end
            end
...
function game_buy_item_scene.getIconAndName(self,shop_sort,value)
    -- "卡牌","装备","道具","能晶"
    local icon,name;
    if shop_sort == 1 then
...
function game_buy_item_scene.getBuySuccessTipsText(self,itemCfg)
    local shop_sort = itemCfg:getNodeWithKey("shop_sort"):toInt();
    local value = itemCfg:getNodeWithKey("value"):toInt();
    ...
    local icon,name = self:getIconAndName(shop_sort,value)
    if shop_sort == 1 then
        text = string_helper.game_buy_item_scene.success_buy  .. get_num .. string_helper.game_buy_item_scene.zhang .. tostring(name) ..  tostring(shopSortTab[shop_sort]);
    elseif shop_sort == 2 then
        text = string_helper.game_buy_item_scene.success_buy .. get_num .. string_helper.game_buy_item_scene.jian .. tostring(name) ..  tostring(shopSortTab[shop_sort]);
    elseif shop_sort == 3 then
        text = string_helper.game_buy_item_scene.success_buy .. get_num .. string_helper.game_buy_item_scene.ge .. tostring(name) ..  tostring(shopSortTab[shop_sort]);
    elseif shop_sort == 4 then
        text = string_helper.game_buy_item_scene.success_buy .. get_num .. string_helper.game_buy_item_scene.ge ..  tostring(shopSortTab[shop_sort]);
    end
    return text;
end
...
function game_buy_item_scene.removePop(self)
...
function game_buy_item_scene.getBuyText(self,itemCfg,bought)
    local need_sort = itemCfg:getNodeWithKey("need_sort"):toInt();
    local need_value = itemCfg:getNodeWithKey("need_value"):toInt();
    local text = "";
    local icon,name = nil,"";
    ...
    text = string_helper.game_buy_item_scene.sure_cost .. cost_value .. tostring(needSortTab[need_sort]) .. string_helper.game_buy_item_scene.buy .. name;
    return text;
end
--[[--

]]
function game_buy_item_scene.addBuyPop(self,shopId)
    local shopCfg = getConfig(game_config_field.shop);
...
function game_buy_item_scene.refreshUi(self)
...
function game_buy_item_scene.refresh4In1(self)
    self.other_node:removeAllChildrenWithCleanup(true)
    local tableView = game_util:setGachaSelect(self.other_node:getContentSize(),3)
    self.other_node:addChild(tableView)
end
--[[--
    初始化
]]
function game_buy_item_scene.init(self,t_params)
...
function game_buy_item_scene.create(self,t_params)
    self:init(t_params);
