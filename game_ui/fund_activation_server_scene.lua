...
local fund_activation_server_scene = {
    m_tGameData = nil,
    m_ccbNode = nil,
...
function fund_activation_server_scene.destroy(self)
    -- body
    cclog("-----------------fund_activation_server_scene destroy-----------------");
...
function fund_activation_server_scene.back(self,type)
    game_scene:enterGameUi("game_main_scene");
end
--[[--
    读取ccbi创建ui
]]
function fund_activation_server_scene.createUi(self)
    ...
    local function onMainBtnClick( target,event )
        ...
        elseif btnTag >= 11 and btnTag <= 13 then--基金
            ...
            local params = {};
            params.id = selFundId;
            network.sendHttpRequest(responseMethod,game_url.getUrlForKey("server_foundation_withdraw"), http_request_method.GET, params,"server_foundation_withdraw")
        end
    end
...
function fund_activation_server_scene.initLayerTouch(self,formation_layer)
    local realPos = nil;
...
function fund_activation_server_scene.setCountdownTime(self)
    ...
    self.m_day_label:setString(tostring(distance_day) .. string_helper.fund_activation_server_scene.text)
end
...
function fund_activation_server_scene.refreshUi(self)
                        ...
                        if i > reward_day then
                            ...
                            if i < value1 then
                                ...
                                if fund_get_reward then
                                    ...
                                    local tempLabel = game_util:createLabelTTF({text = string_helper.fund_activation_server_scene.text2,color = ccc3(255,0,0),fontSize = 14})
...
function fund_activation_server_scene.init(self,t_params)
...
function fund_activation_server_scene.create(self,t_params)
    -- body
    self:init(t_params);
...
return fund_activation_server_scene;