---  幸运转盘
local game_lucky_turntable_server = {
    m_root_layer = nil,
    m_popUi = nil,
    m_close_btn = nil,
    m_list_view_bg = nil,
...
function game_lucky_turntable_server.destroy(self)
    -- body
    cclog("----------------- game_lucky_turntable_server destroy-----------------"); 
...
function game_lucky_turntable_server.back(self,backType)
    -- game_scene:removePopByName("game_lucky_turntable_server");
...
function game_lucky_turntable_server.createUi(self)
    ...
    local function onBtnClick( target,event )
        ...
        elseif btnTag == 12 then--查看详情
            game_scene:addPop("game_active_limit_detail_pop",{openType = "game_lucky_turntable_server",enterType = self.m_tGameData.version})
        elseif btnTag == 101 then--启动一次
            ...
            network.sendHttpRequest(responseMethod,game_url.getUrlForKey("server_roulette_open_roulette"), http_request_method.GET, {},"server_roulette_open_roulette")
        elseif btnTag == 102 then--启动10次
            ...
            network.sendHttpRequest(responseMethod,game_url.getUrlForKey("server_roulette_open_roulette10"), http_request_method.GET, {},"server_roulette_open_roulette10")
        elseif btnTag == 103 then--查看其他奖励
            game_scene:addPop("game_lucky_turntable_reward_server_pop",{version = self.m_tGameData.version})
        elseif btnTag == 104 then--刷新
            ...
            network.sendHttpRequest(responseMethod,game_url.getUrlForKey("server_roulette_refresh"), http_request_method.GET, {},"server_roulette_refresh")
        end
    end
...
function game_lucky_turntable_server.createOneReward(self,rewardTab,rewardTurntableType)
    -- assert(false)
...
function game_lucky_turntable_server.addOneAnim(self,quality)
...
function game_lucky_turntable_server.addTenAnim(self,index,rewardTab)
    if index == 1 then
...
function game_lucky_turntable_server.removeGachaAnim(self)
    self.m_mask_layer:removeAllChildrenWithCleanup(true);
    self.m_gachaAnimTab = {};
end
...
function game_lucky_turntable_server.refreshLabel(self)
    ...
    local function timeEndFunc()
       self.left_time_node:removeAllChildrenWithCleanup(true)
       local tipsLabel = game_util:createLabelTTF({text = string_helper.game_lucky_turntable_server.text,color = ccc3(0,255,0),fontSize = 10});
    ...
    if roulette_cfg_item then
        self.m_ten_cost_label:setString(string.format(string_helper.game_lucky_turntable_server.diamond,roulette_cfg_item:getNodeWithKey("price_10"):toInt()))
        ...
        if open_times > 0 then
            self.m_residue_degree_label:setString(string.format(string_helper.game_lucky_turntable_server.reside,open_times));
        else
            self.m_residue_degree_label:setString(string.format(string_helper.game_lucky_turntable_server.diamond,roulette_cfg_item:getNodeWithKey("price"):toInt()));
        end
        local refresh_times = self.m_tGameData.refresh_times or 0
        if refresh_times > 0 then
            self.m_refresh_count_label:setString(string.format(string_helper.game_lucky_turntable_server.reside,refresh_times));
        else
            self.m_refresh_count_label:setString(string.format(string_helper.game_lucky_turntable_server.diamond,roulette_cfg_item:getNodeWithKey("refresh_price"):toInt()));
        end
    else
        -- self.m_active_time_label:setString("");
        self.m_residue_degree_label:setString(string.format(string_helper.game_lucky_turntable_server.diamond, 50));
        self.m_ten_cost_label:setString(string.format(string_helper.game_lucky_turntable_server.diamond,480))
    end
...
function game_lucky_turntable_server.createLuckyTurntableReward(self)
...
function game_lucky_turntable_server.initLayerTouch(self,formation_layer)
    local realPos = nil;
...
function game_lucky_turntable_server.createRotationAnim(self,turntableType)
...
function game_lucky_turntable_server.createTableView(self,viewSize)
...
function game_lucky_turntable_server.createRewardTabelView(self,viewSize)
    local roulette_rank_reward_cfg = getConfig(game_config_field.server_roulette_rank_reward)
    local version = tostring(self.m_tGameData.version or "");
    local tempCount = roulette_rank_reward_cfg:getNodeCount();
    local showData = {};
    for i=1,tempCount do
        local itemCfg = roulette_rank_reward_cfg:getNodeAt(i - 1);
        local team = itemCfg:getNodeWithKey("team"):toStr();
    ...
    local itemCfg = roulette_rank_reward_cfg:getNodeWithKey(tostring(showData[1]));
    local rewardTab = {};
    if itemCfg then
...
function game_lucky_turntable_server.createRewardTabelView2(self,viewSize)
...
function game_lucky_turntable_server.refreshTableView(self)
...
function game_lucky_turntable_server.refreshRewardTableView(self)
   ...
   self.reward_table_node2:addChild(tempRewardTable,10,10)
end
--[[--
    刷新ui
]]
function game_lucky_turntable_server.refreshUi(self)
...
function game_lucky_turntable_server.init(self,t_params)
...
function game_lucky_turntable_server.create(self,t_params)
    self:init(t_params);
...
function game_lucky_turntable_server.gameGuide(self,guideType,guide_team,guide_id,t_params)
...
return game_lucky_turntable_server;
