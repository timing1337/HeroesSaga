...
function game_sky_star_server.destroy(self)
    -- body
    cclog("----------------- game_sky_star_server destroy-----------------"); 
...
function game_sky_star_server.back(self,backType)
...
function game_sky_star_server.createUi(self)
    ...
    local function onBtnClick( target,event )
        ...
        elseif btnTag == 12 then--查看详情
            game_scene:addPop("game_active_limit_detail_pop",{openType = "game_sky_star_server",cfgKey = "905"})
        elseif btnTag == 13 then--启动
            ...
            -- reward_id = 1
            network.sendHttpRequest(responseMethod,game_url.getUrlForKey("server_bandit_get_reward"), http_request_method.GET, {},"server_bandit_get_reward")
        end
    end
...
function game_sky_star_server.createNumberUi(self)
...
function game_sky_star_server.runNumberAction(self,index,toNumber,delayTime,actionCallBackFunc,lastFlag)
...
function game_sky_star_server.startActiveAnim(self)
...
function game_sky_star_server.refreshLabel(self)
    --倒计时
    self.m_left_time_node:removeAllChildrenWithCleanup(true)
    local function timeEndFunc()
       self.m_left_time_node:removeAllChildrenWithCleanup(true)
       local tipsLabel = game_util:createLabelTTF({text = string_helper.game_sky_star_server.out,color = ccc3(255,255,255),fontSize = 10});
    ...
    local bandit_cfg = getConfig(game_config_field.server_bandit)
    ...
    local itemCfg = bandit_cfg:getNodeWithKey(tostring(tonumber(currKey) + 1))
    if itemCfg then
        ...
        self.m_get_coin_label:setString(string_helper.game_sky_star_server.sd .. value_down .. " - " .. value_up)
        self.m_cost_coin_label:setString(string_helper.game_sky_star_server.cost .. coin);
    else
        self.m_get_coin_label:setString(string_helper.game_sky_star_server.sd1)
        self.m_cost_coin_label:setString(string_helper.game_sky_star_server.out_lt);
    end
end
...
function game_sky_star_server.createTableView(self,viewSize)
    ...
    params.newCell = function(tableView,index) 
        ...
        if cell then
            ...
            local richLabel = game_util:createRichLabelTTF({text = "[color=ffffdc00]" .. tostring(itemData.name) .. string_helper.game_sky_star_server.player .. tostring(itemData.coin) .. string_helper.game_sky_star_server.diamond,dimensions = itemSize,textAlignment = kCCTextAlignmentLeft,
...
function game_sky_star_server.refreshTableView(self)
...
function game_sky_star_server.refreshUi(self)
...
function game_sky_star_server.init(self,t_params)
...
function game_sky_star_server.create(self,t_params)
    self:init(t_params);
...
function game_sky_star_server.gameGuide(self,guideType,guide_team,guide_id,t_params)

end

return game_sky_star_server;
