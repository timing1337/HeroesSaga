...
function game_boss_rank.destroy(self)
    cclog("-----------------game_boss_rankdestroy-----------------");
    self.m_gameData = nil;
    --排行榜的控件
...
function game_boss_rank.back(self,backType)
...
function game_boss_rank.createUi(self)
...
function game_boss_rank.createRankTableView(self,viewSize, data)
    ...
    params.newCell = function(tableView,index) 
        ...
        if cell then
            ...
            guild_name_label:setString(tostring(user_info.guild_name or string_helper.game_boss_rank.wu))  
            for i=1,4 do
        ...
        fight_count_label:setString(tostring(user_info.score));
            ...
            if index == fightCount - 1 and self.combat_page < 4 then
                local tipLabel = game_util:createLabelTTF({text = string_helper.game_boss_rank.down_refresh,color = ccc3(250,180,0),fontSize = 12});
...
function game_boss_rank.refreshUi(self)
    self:refreshTabByData(self.userCombat);
end
--[[
    刷新表2
]]
function game_boss_rank.refreshTabByData(self, data)
...
function game_boss_rank.init(self,t_params)
...
function game_boss_rank.create(self,t_params)
    self:init(t_params);
